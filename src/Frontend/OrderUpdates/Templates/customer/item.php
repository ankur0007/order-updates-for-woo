<?php
/**
 * Single update item on the customer-facing order updates list. Rendered
 * once per update by CustomerOrderUpdatesView from inside each section
 * (Active / Archived).
 *
...
$is_open_by_default = $is_first_in_section && ! $is_resolved;
		...
		if ( $has_more_notes && ! empty( $notes ) ) :
			...
			<?php else : ?>
				...
				foreach ( $notes as $note ) :
					...
					$avatar_url  = (string) ( $note['avatar_url'] ?? '' );
					?>
